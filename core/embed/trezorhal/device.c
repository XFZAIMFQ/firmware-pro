...
void device_verify_ble(void) {
    ...
    if ( secfalse == flash_otp_is_locked(FLASH_OTP_BLOCK_BLE_PUBKEY1) ||
        ...
        // 烧写BLE公钥到STM_OTP
        ensure(ble_get_pubkey(pubkey + 1) ? sectrue : secfalse, NULL); // 获取BLE公钥
        ensure(ble_lock_pubkey() ? sectrue : secfalse, NULL); // 锁定公钥
    ...
    pubkey[0] = 0x04;
    random_buffer(rand_buf, sizeof(rand_buf)); // 生成随机数
    ensure(ble_sign_msg(rand_buf, sizeof(rand_buf), signature) ? sectrue : secfalse, NULL); // 签名随机数
    sha256_Raw(rand_buf, 16, digest); // 计算随机数的SHA256摘要

    ensure(ecdsa_verify_digest(&secp256k1, pubkey, signature, digest) == 0 ? sectrue : secfalse, NULL); // 验证签名
}

/**
 * @brief 设备参数初始化
 *
 * 初始化设备的唯一ID、公钥和随机密钥等参数。
 *
 * 如果公钥尚未锁定，则从安全元件获取公钥并将其写入OTP存储区，然后锁定该区域以防止进一步修改。
 */
void device_para_init(void) {
    dev_info.st_id[0] = HAL_GetUIDw0(); // 获取设备唯一ID_0
    dev_info.st_id[1] = HAL_GetUIDw1(); // 获取设备唯一ID_1
    dev_info.st_id[2] = HAL_GetUIDw2(); // 获取设备唯一ID_2
    ...
    if ( secfalse == flash_otp_is_locked(FLASH_OTP_BLOCK_THD89_1_PUBKEY1) ||
        ...
        ensure(se_get_ecdh_pubkey(THD89_MASTER_ADDRESS, pubkey), NULL); // 获取ecdh公钥
        ensure(se_lock_ecdh_pubkey(THD89_MASTER_ADDRESS), NULL);        // 锁定公钥
        ensure(flash_otp_write(FLASH_OTP_BLOCK_THD89_1_PUBKEY1, 0, pubkey, FLASH_OTP_BLOCK_SIZE), NULL);
        ensure(flash_otp_write(FLASH_OTP_BLOCK_THD89_1_PUBKEY2, 0, pubkey + 32, FLASH_OTP_BLOCK_SIZE), NULL);
        ensure(flash_otp_lock(FLASH_OTP_BLOCK_THD89_1_PUBKEY1), NULL); // 锁定OTP存储区
        ensure(flash_otp_lock(FLASH_OTP_BLOCK_THD89_1_PUBKEY2), NULL); // 锁定OTP存储区
    }
    // 烧写THD89_2公钥到STM_OTP
    if ( secfalse == flash_otp_is_locked(FLASH_OTP_BLOCK_THD89_2_PUBKEY1) ||
        ...
        ensure(se_get_ecdh_pubkey(THD89_2ND_ADDRESS, pubkey), NULL); // 获取ecdh公钥
        ensure(se_lock_ecdh_pubkey(THD89_2ND_ADDRESS), NULL);        // 锁定公钥
        ensure(flash_otp_write(FLASH_OTP_BLOCK_THD89_2_PUBKEY1, 0, pubkey, FLASH_OTP_BLOCK_SIZE), NULL); 
        ensure(flash_otp_write(FLASH_OTP_BLOCK_THD89_2_PUBKEY2, 0, pubkey + 32, FLASH_OTP_BLOCK_SIZE), NULL);
    ...
    if ( secfalse == flash_otp_is_locked(FLASH_OTP_BLOCK_THD89_3_PUBKEY1) ||
        ...
        ensure(flash_otp_write(FLASH_OTP_BLOCK_THD89_3_PUBKEY1, 0, pubkey, FLASH_OTP_BLOCK_SIZE), NULL);
        ensure(flash_otp_write(FLASH_OTP_BLOCK_THD89_3_PUBKEY2, 0, pubkey + 32, FLASH_OTP_BLOCK_SIZE), NULL);
    ...
    if ( secfalse == flash_otp_is_locked(FLASH_OTP_BLOCK_THD89_4_PUBKEY1) ||
        ...
        ensure(flash_otp_write(FLASH_OTP_BLOCK_THD89_4_PUBKEY1, 0, pubkey, FLASH_OTP_BLOCK_SIZE), NULL);
        ensure(flash_otp_write(FLASH_OTP_BLOCK_THD89_4_PUBKEY2, 0, pubkey + 32, FLASH_OTP_BLOCK_SIZE), NULL);
    ...
    if ( flash_otp_is_locked(FLASH_OTP_CPU_FIRMWARE_INFO) ) {
        strlcpy(dev_info.cpu_info, (char*)flash_otp_data->flash_otp[FLASH_OTP_CPU_FIRMWARE_INFO], sizeof(dev_info.cpu_info));
        strlcpy(
...
void ui_test_input(void) {
    ...
    for ( int i = 0; i < 5; i++ ) {
        for ( int j = 0; j < 6; j++ ) {
            display_bar_radius(j * 80, (j % 2) * 80 + i * 160, 80, 80, COLOR_RED, COLOR_WHITE, 16);
        }
    }
    uint32_t pos = 0;
    for ( ;; ) {
        ...
        for ( int i = 0; i < 5; i++ ) {
            for ( int j = 0; j < 6; j++ ) {
                if ( x > (j * 80) && x < (j * 80 + 80) && y > ((j % 2) * 80 + i * 160) &&
                     y < ((j % 2) * 80 + i * 160 + 80) ) {
                    display_bar_radius(j * 80, (j % 2) * 80 + i * 160, 80, 80, COLOR_GREEN, COLOR_WHITE, 16);
                    pos |= 1 << (6 * i + j);
                }
                if ( pos == 0x3FFFFFFF ) {
                    return;
...
void device_burnin_test_clear_flag(void) {
    ...
    if ( res != FR_OK ) {
        display_text_center(
            DISPLAY_RESX / 2, DISPLAY_RESY / 2, "mount fatfs failed", -1, FONT_NORMAL, COLOR_RED, COLOR_BLACK
        );
        while ( 1 )
            ;
    }
...
void device_generate_trng_data(void) {
    ...
    while ( batch_current <= batch_total ) {
        // ui init
        snprintf(note_buf, sizeof(note_buf), "Batch   %u / %u", batch_current, batch_total);
        display_clear();
        ui_screen_progress_bar_prepare(title_buf, note_buf);

        // path
        snprintf(path_buf, sizeof(path_buf), "0:TRNG_Test_Data/batch_%u.bin", batch_current);
        ...
        while ( batch_processed_bytes < batch_total_bytes ) {
            ...
            ensure_emmcfs(
                emmc_fs_file_write(
                    path_buf, batch_processed_bytes, se_rand_buffer, sizeof(se_rand_buffer), &processed_len, false, true
                ),
                "file write failed"
            );
            ...
            // ui update
            ui_screen_progress_bar_update(NULL, NULL, (batch_processed_bytes * 100 / batch_total_bytes));
...
static void lock_burnin_test_otp(void) {
    ...
    ensure(flash_otp_write(FLASH_OTP_BLOCK_BURNIN_TEST, 0, buf, FLASH_OTP_BLOCK_SIZE), NULL);
...
static void ui_generic_confirm_simple(const char* msg) {
    ...
    display_text_center(DISPLAY_RESX / 2, DISPLAY_RESY / 2, msg, -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK);

    display_bar_radius(32, DISPLAY_RESY - 160, 128, 64, COLOR_RED, COLOR_BLACK, 16);
    display_bar_radius(DISPLAY_RESX - 32 - 128, DISPLAY_RESY - 160, 128, 64, COLOR_GREEN, COLOR_BLACK, 16);
    display_text(80, DISPLAY_RESY - 120, "No", -1, FONT_NORMAL, COLOR_WHITE, COLOR_RED);
    display_text(DISPLAY_RESX - 118, DISPLAY_RESY - 120, "Yes", -1, FONT_NORMAL, COLOR_WHITE, COLOR_GREEN);
}
...
static bool ui_response(void) {
    for ( ;; ) {
        ...
        if ( x >= 32 && x < 32 + 128 && y > DISPLAY_RESY - 160 && y < DISPLAY_RESY - 160 + 64 ) {
            return false;
        }
        // clicked on Confirm button
        if ( x >= DISPLAY_RESX - 32 - 128 && x < DISPLAY_RESX - 32 && y > DISPLAY_RESY - 160 &&
...
static bool __attribute__((unused)) _motor_test(void) {
    int ui_res = 0;
    motor_init();
...
static bool _camera_test(void) {
    ...
    while ( 1 ) {
        ...
        if ( camera_capture_done() ) {
            dma2d_copy_buffer((uint32_t*)CAM_BUF_ADDRESS, (uint32_t*)lcd_get_src_addr(), 80, 240, WIN_W, WIN_H);
        }
...
static bool __attribute__((unused)) _nfc_test() {
    int ui_res = 0;
    display_bar(0, 380, DISPLAY_RESX, 420, COLOR_BLACK);
    display_text_center(DISPLAY_RESX / 2, DISPLAY_RESY / 2, "NFC test", -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK);
    ...
    while ( 1 ) {
        if ( nfc_poll_card() ) {
            ...
            if ( nfc_select_aid((uint8_t*)"\x6f\x6e\x65\x6b\x65\x79\x2e\x62\x61\x63\x6b\x75\x70\x01", 14) ) {
                if ( lite_card_data_exchange_test() ) {
                    nfc_pwr_ctl(false);
                    return true;
                }
...
void device_test(bool force) {
    ...
    //     display_text(0, 20, "SE test faild", -1, FONT_NORMAL, COLOR_RED, COLOR_BLACK);
    //     while ( 1 )
    //         ;
    // } else {
    //     display_text(0, 20, "SE test done", -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK);
    // }

    if ( get_hw_ver() < HW_VER_3P0A ) {
        if ( qspi_flash_read_id() == 0 ) {
            display_text(0, 50, "SPI-FLASH test faild", -1, FONT_NORMAL, COLOR_RED, COLOR_BLACK);
            while ( 1 )
                ;
        } else {
            display_text(0, 50, "SPI-FLASH test done", -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK);
        }
    }

    if ( emmc_get_capacity_in_bytes() == 0 ) {
        display_text(0, 80, "EMMC test faild", -1, FONT_NORMAL, COLOR_RED, COLOR_BLACK);
        while ( 1 )
            ;
    } else {
        display_text(0, 80, "EMMC test done", -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK);
    }

    // if ( _motor_test() ) {
    //     display_text(0, 110, "MOTOR test done", -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK);
    // } else {
    //     display_text(0, 110, "MOTOR test faild", -1, FONT_NORMAL, COLOR_RED, COLOR_BLACK);
    //     while ( 1 )
    //         ;
    // }

    if ( _camera_test() ) {
        display_text(0, 140, "CAMERA test done", -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK);
    } else {
        display_text(0, 140, "CAMERA test faild", -1, FONT_NORMAL, COLOR_RED, COLOR_BLACK);
        while ( 1 )
            ;
    }

    // if ( _nfc_test() ) {
    //     display_text(0, 170, "NFC test done", -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK);
    // } else {
    //     display_text(0, 170, "NFC test faild", -1, FONT_NORMAL, COLOR_RED, COLOR_BLACK);
    //     while ( 1 )
    //         ;
    // }

    if ( _fp_test() ) {
        display_text(0, 200, "FINGERPRINT test done", -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK);
    } else {
        display_text(0, 200, "FINGERPRINT test faild", -1, FONT_NORMAL, COLOR_RED, COLOR_BLACK);
        while ( 1 )
            ;
    }

    if ( _flashled_test() ) {
        display_text(0, 230, "FLASHLED test done", -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK);
    } else {
        display_text(0, 230, "FLASHLED test faild", -1, FONT_NORMAL, COLOR_RED, COLOR_BLACK);
        while ( 1 )
            ;
    }
    ...
    // ensure(flash_otp_write(FLASH_OTP_FACTORY_TEST, 0, buf, FLASH_OTP_BLOCK_SIZE), NULL);
...
void device_burnin_test(bool force) {
    ...
    static uint16_t voltage_last = 0, current_last = 0, discahrging_current_last = 0;
    ...
    if ( flash_otp_is_locked(FLASH_OTP_BLOCK_BURNIN_TEST) && (test_res.touch == TEST_PASS) ) {
        return;
    }
    ...
    do {
        ...
        if ( previous_remain - remain >= (TIMER_1S / 2) ) {
            ...
            mini_snprintf(remain_timer, sizeof(remain_timer), "%02d:%02d:%02d", hour, min, sec);
            display_bar(DISPLAY_RESX / 2 - w / 2, 770, w, 30, COLOR_BLACK);
            display_text_center(DISPLAY_RESX / 2, 800, remain_timer, -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK);
        }
        ...
        if ( index != index_bak ) {
            ...
            case 0:
                ...
                if ( jped_decode("res/wallpaper-1.jpg", FMC_SDRAM_LVGL_BUFFER_ADDRESS) != 0 ) {
                    display_print_clear();
                    display_printf("show wallpaper-1.jpg err\n");
                }
                break;
            case 1:
                if ( jped_decode("res/wallpaper-2.jpg", FMC_SDRAM_LVGL_BUFFER_ADDRESS) != 0 ) {
                    display_print_clear();
                    display_printf("show wallpaper-2.jpg err\n");
                }
                break;
            case 2:
                if ( jped_decode("res/wallpaper-3.jpg", FMC_SDRAM_LVGL_BUFFER_ADDRESS) != 0 ) {
                    display_print_clear();
        ...
        if ( (current - previous) > (TIMER_1S / 2) ) {
            ...
            if ( emmc_cap == 0 ) {
                display_text_center(
                    DISPLAY_RESX / 2, DISPLAY_RESY / 2, "EMMC test faild", -1, FONT_NORMAL, COLOR_RED, COLOR_BLACK
                );
                while ( 1 )
                    ;
            }
            ...
            if ( !se_get_rand(rand_buffer, 32) ) {
                display_text_center(
                    DISPLAY_RESX / 2, DISPLAY_RESY / 2, "SE test faild", -1, FONT_NORMAL, COLOR_RED, COLOR_BLACK
                );
                while ( 1 )
                    ;
            }
  ...
  #define FONT_HEIGHT 25
        ...
        if ( battery_now - battery_pre > 1000 ) {
            ...
            if ( voltage_last != voltage ) {
                ...
                mini_snprintf(battery_str, sizeof(battery_str), "voltage %d mv", (voltage_last));
                display_bar(0, battery_info_offset_y - 20, 360, FONT_HEIGHT, COLOR_BLACK);
                display_text(0, battery_info_offset_y, battery_str, -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK);
            }
            ...
            if ( current_last != current ) {
                ...
                mini_snprintf(battery_str, sizeof(battery_str), "charging current %d ma", (current_last));
                display_bar(0, battery_info_offset_y - 20, 360, FONT_HEIGHT, COLOR_BLACK);
                display_text(0, battery_info_offset_y, battery_str, -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK);
            }
            ...
            if ( discahrging_current_last != discharging_current ) {
                ...
                mini_snprintf(
                    battery_str, sizeof(battery_str), "discharging current %d ma", (discahrging_current_last)
                );
                display_bar(0, battery_info_offset_y - 20, 360, FONT_HEIGHT, COLOR_BLACK);
                display_text(0, battery_info_offset_y, battery_str, -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK);
            }
            ...
            if ( !ble_charging_state() ) {
                ...
                if ( charge_type_last != charge_type ) {
                    ...
                    if ( CHARGE_TYPE_USB == charge_type_last ) {
                        display_text(
                            0, battery_info_offset_y, "charging via usb", -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK
                        );
                    } else if ( CHARGE_TYPE_WIRELESS == charge_type_last ) {
                        display_text(
                            0,
                            battery_info_offset_y,
                        ...
                        display_text(0, battery_info_offset_y, "uncharged", -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK);
                    }
                }
            }
            battery_info_offset_y += FONT_HEIGHT;
            ...
            if ( ble_get_battery_inner_temp(&battery_temp) ) {
                mini_snprintf(battery_str, sizeof(battery_str), "battery temp %d c", (battery_temp));
                display_bar(0, battery_info_offset_y - 20, 360, FONT_HEIGHT, COLOR_BLACK);
                display_text(0, battery_info_offset_y, battery_str, -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK);
            }
        }
        ...
        if ( index == 3 ) {
            ...
            camera_capture_start();
            if ( camera_capture_done() ) {
                dma2d_copy_buffer((uint32_t*)CAM_BUF_ADDRESS, (uint32_t*)lcd_get_src_addr(), 80, 300, WIN_W, WIN_H);
            }
        }
...
bool device_overwrite_serial(char* dev_serial) {
    ...
    // change and write back
    memset(otp_data_buffer_p->flash_otp[FLASH_OTP_DEVICE_SERIAL], 0x00, FLASH_OTP_BLOCK_SIZE);
