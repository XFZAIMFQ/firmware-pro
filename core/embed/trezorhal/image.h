...
#define FIRMWARE_IMAGE_INNER_SIZE            (FIRMWARE_INNER_SECTORS_COUNT * IMAGE_CHUNK_SIZE)
#define FIRMWARE_IMAGE_MAXSIZE               (FIRMWARE_IMAGE_INNER_SIZE + FMC_SDRAM_FIRMWARE_P2_LEN)
...
secbool __wur load_image_header(
...
secbool __wur
load_ble_image_header(const uint8_t* const data, const uint32_t magic, const uint32_t maxsize, image_header* const hdr);

secbool __wur load_thd89_image_header(
    const uint8_t* const data,
    const uint32_t magic,
    const uint32_t maxsize,
...
secbool __wur load_vendor_header(
    const uint8_t* const data,
    uint8_t key_m,
    uint8_t key_n,
    const uint8_t* const* keys,
...
secbool __wur
check_image_contents(const image_header* const hdr, uint32_t firstskip, const uint8_t* sectors, int blocks);

secbool __wur check_image_contents_ADV(
...
secbool __wur verify_bootloader(
    image_header* const hdr,
...
secbool __wur install_firmware(
    const uint8_t* const buffer,
    const size_t size,
    char* error_msg,
    size_t error_msg_len,
