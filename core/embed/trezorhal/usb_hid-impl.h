...
int usb_hid_read(uint8_t iface_num, uint8_t* buf, uint32_t len) {
    ...
    USBD_LL_PrepareReceive(&usb_dev_handle, state->ep_out, state->rx_buffer, state->max_packet_len);
    ...
    case USB_REQ_TYPE_STANDARD:
        ...
        case USB_REQ_GET_DESCRIPTOR:
            ...
            case USB_DESC_TYPE_HID:
                USBD_CtlSendData(
                    dev, UNCONST(&state->desc_block->hid), MIN_8bits(req->wLength, sizeof(state->desc_block->hid))
                );
                return USBD_OK;
