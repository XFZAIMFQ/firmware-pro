...
static int sdram_init_sequence(void) {
    ...
    /* Send the command */
    if ( HAL_SDRAM_SendCommand(&hsdram[0], &Command, FMC_SDRAM_TIMEOUT) != HAL_OK ) {
        return HAL_ERROR;
    }
    ...
    tmpmrd = (uint32_t)FMC_SDRAM_DEVICE_BURST_LENGTH_1 | FMC_SDRAM_DEVICE_BURST_TYPE_SEQUENTIAL |
             FMC_SDRAM_DEVICE_CAS_LATENCY_2 | FMC_SDRAM_DEVICE_OPERATING_MODE_STANDARD |
...
int sdram_init(void) {
    ...
    /* GPIOG configuration */
    gpio_init_structure.Pin =
        GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_2 /*| GPIO_PIN_3 */ | GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_15;
    HAL_GPIO_Init(GPIOG, &gpio_init_structure);

    /* GPIOH configuration */
    gpio_init_structure.Pin = GPIO_PIN_5 | GPIO_PIN_6 | GPIO_PIN_7 | GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_10 |
                              GPIO_PIN_11 | GPIO_PIN_12 | GPIO_PIN_13 | GPIO_PIN_14 | GPIO_PIN_15;
    ...
    hsdram[0].Init.SDBank = FMC_SDRAM_BANK2;                             // bank2
    hsdram[0].Init.ColumnBitsNumber = FMC_SDRAM_COLUMN_BITS_NUM_9;       // 9列
    hsdram[0].Init.RowBitsNumber = FMC_SDRAM_ROW_BITS_NUM_12;            // 12行
    hsdram[0].Init.MemoryDataWidth = FMC_SDRAM_MEM_BUS_WIDTH_32;         // 32位宽
    hsdram[0].Init.InternalBankNumber = FMC_SDRAM_INTERN_BANKS_NUM_4;    // 4个内部bank
    hsdram[0].Init.CASLatency = FMC_SDRAM_CAS_LATENCY_2;                 // CAS延迟2
    hsdram[0].Init.WriteProtection = FMC_SDRAM_WRITE_PROTECTION_DISABLE; // 禁止写保护
    hsdram[0].Init.SDClockPeriod = FMC_SDRAM_CLOCK_PERIOD_2;             // 时钟周期2
    hsdram[0].Init.ReadBurst = FMC_SDRAM_RBURST_ENABLE;                  // 读突发使能
    hsdram[0].Init.ReadPipeDelay = FMC_SDRAM_RPIPE_DELAY_0;              // 读管道延迟0
...
int sdram_reinit(void) {
    ...
    HAL_GPIO_DeInit(
        GPIOG,
        GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_2 /*| GPIO_PIN_3 */ | GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_8 | GPIO_PIN_15
    );
