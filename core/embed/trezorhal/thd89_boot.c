...
static bool _se_get_state(uint8_t addr, uint8_t* state) {
    ...
    if ( (resp_len != 0x01) || ((state[0] != 0x00) && (state[0] != 0x55) && (state[0] != 0x33)) ) {
        return false;
    }
    return true;
}
...
char* se_get_version_ex(void) {
    ...
    if ( !thd89_transmit_ex(device_addr, get_ver, sizeof(get_ver), (uint8_t*)ver, &ver_len) ) {
        return NULL;
    }
...
bool se_update_firmware(
    ...
    while ( data_len ) {
        ...
        if ( ui_callback ) {
            ui_callback(percent_start + weights * offset_len / (offset_len + data_len));
        }
    }
