...
static secbool
compute_pubkey(uint8_t sig_m, uint8_t sig_n, const uint8_t* const* pub, uint8_t sigmask, ed25519_public_key res) {
    if ( 0 == sig_m || 0 == sig_n )
        return secfalse;
    if ( sig_m > sig_n )
        return secfalse;
    ...
    if ( __builtin_popcount(sigmask) != sig_m )
        return secfalse;
...
secbool load_image_header(
    ...
    if ( hdr->codelen > (maxsize - hdr->hdrlen) )
        return secfalse;
    if ( (hdr->hdrlen + hdr->codelen) < 4 * 1024 )
        return secfalse;
    if ( (hdr->hdrlen + hdr->codelen) % 512 != 0 )
        return secfalse;
    ...
    return sectrue *
           (0 == ed25519_sign_open(hdr->fingerprint, BLAKE2S_DIGEST_LENGTH, pub, *(const ed25519_signature*)hdr->sig));
}

secbool load_ble_image_header(
    const uint8_t* const data,
    const uint32_t magic,
    const uint32_t maxsize,
    image_header* const hdr
...
secbool load_thd89_image_header(
    const uint8_t* const data,
    const uint32_t magic,
    const uint32_t maxsize,
...
/**
 * @brief 读取 vendor header
 * 
 * @param data 
 * @param vhdr 
 * @return secbool 
 */
secbool read_vendor_header(const uint8_t* const data, vendor_header* const vhdr) {
    memcpy(&vhdr->magic, data, 4);
    if ( vhdr->magic != 0x56544B4F )
        return secfalse; // OKTV

    memcpy(&vhdr->hdrlen, data + 4, 4); // 读取vendor header len
    if ( vhdr->hdrlen > 64 * 1024 )
        return secfalse;
...
secbool check_image_contents(const image_header* const hdr, uint32_t firstskip, const uint8_t* sectors, int blocks) {
    ...
    if ( remaining <= IMAGE_CHUNK_SIZE - firstskip ) {
        if ( sectrue != check_single_hash(hdr->hashes, data, MIN(remaining, IMAGE_CHUNK_SIZE)) ) {
            return secfalse;
        } else {
            return sectrue;
        }
    ...
    while ( remaining > 0 ) {
        ...
        if ( remaining - IMAGE_CHUNK_SIZE > 0 ) {
            if ( block % 2 ) {
                ...
                blake2s_Final(&ctx, hash, BLAKE2S_DIGEST_LENGTH);
                if ( 0 != memcmp(hdr->hashes + (block / 2) * 32, hash, BLAKE2S_DIGEST_LENGTH) ) {
                    return secfalse;
                }
            } else {
                blake2s_Init(&ctx, BLAKE2S_DIGEST_LENGTH);
            ...
            if ( update_flag ) {
                ...
                if ( sectrue !=
                     check_single_hash(hdr->hashes + (block / 2) * 32, data, MIN(remaining, IMAGE_CHUNK_SIZE)) ) {
                    return secfalse;
                }
            }
        }
...
secbool check_image_contents_ADV(
    ...
    const size_t first_chunk_skip = ((vhdr != NULL) ? vhdr->hdrlen : 0) + hdr->hdrlen;
    ...
    while ( true ) {
        if ( processed_size == 0 && block == 0 )
            process_size = MIN((code_len_check - processed_size), (hash_chunk_size - first_chunk_skip));
        else
            process_size = MIN((code_len_check - processed_size), (hash_chunk_size));

        if ( sectrue == check_single_hash(hdr->hashes + block * 32, code_buffer + processed_size, process_size) ) {
            block++;
...
secbool install_bootloader(
    ...
    while ( processed_size < size ) {
        ...
        EXEC_RETRY(
            ...
            sectrue,
            {},
            { return flash_erase(BOOTLOADER_SECTORS[processed_size / FLASH_BOOTLOADER_SECTOR_SIZE]); },
            {},
            {
                strncpy(error_msg, "Flash bootloader area erase failed!", error_msg_len);
                return secfalse;
            }
        );
        ...
        // write
        for ( size_t sector_offset = 0; sector_offset < FLASH_BOOTLOADER_SECTOR_SIZE; sector_offset += 32 ) {
            ...
            EXEC_RETRY(
                ...
                {},
                {
                    return flash_write_words(
                        BOOTLOADER_SECTORS[processed_size / FLASH_BOOTLOADER_SECTOR_SIZE],
                        sector_offset,
                        (uint32_t*)(buffer + processed_size)
                    );
                },
                {},
                {
            ...
            processed_size += ((size - processed_size) > 32) ? 32 // since we could only write 32 byte a time
                                                             : (size - processed_size);
 ...
 *
 * @param hdr image header
 * @param hdr_valid image header 验证结果
 * @param code_valid code 验证结果
 * @param error_msg
 * @param error_msg_len
 * @return secbool
 */
secbool verify_bootloader(
    image_header* const hdr,
    secbool* const hdr_valid,
    secbool* const code_valid,
    char* error_msg,
    size_t error_msg_len
) {
    // internal vars
    ...
    if ( hdr != NULL )
        memset(hdr, 0xff, sizeof(image_header));
    if ( hdr_valid != NULL )
        *hdr_valid = secfalse;
    if ( code_valid != NULL )
        *code_valid = secfalse;
    ...
    ExecuteCheck_ADV(
        load_image_header(
            (const uint8_t*)BOOTLOADER_START,
            BOOTLOADER_IMAGE_MAGIC,
    ...
    _hdr_valid = sectrue;
    if ( hdr != NULL )
        memcpy(hdr, &_hdr, sizeof(image_header));
    if ( hdr_valid != NULL )
        *hdr_valid = _hdr_valid;

    // verify code
    ExecuteCheck_ADV(
        check_image_contents_ADV(NULL, &_hdr, (const uint8_t*)BOOTLOADER_START + _hdr.hdrlen, 0, _hdr.codelen, true),
        sectrue,
        {
            if ( error_msg != NULL )
    ...
    _code_valid = sectrue;
    if ( code_valid != NULL )
        *code_valid = _code_valid;

    return (((_hdr_valid == sectrue) && (_code_valid == sectrue))) ? sectrue : secfalse;
}

secbool install_firmware(
    const uint8_t* const buffer,
    const size_t size,
    char* error_msg,
    size_t error_msg_len,
    ...
    // const vars
    const size_t fw_internal_size = FLASH_FIRMWARE_SECTOR_SIZE * FIRMWARE_INNER_SECTORS_COUNT;
    ...
    // prepare
    if ( !emmc_fs_dir_make("0:/data") || !emmc_fs_file_delete("0:/data/fw_p2.bin") ) {
    ...
    while ( processed_size < size ) {
        if ( processed_size < fw_internal_size ) {
            ...
            EXEC_RETRY(
                ...
                sectrue,
                {},
                { return flash_erase(FIRMWARE_SECTORS[processed_size / FLASH_FIRMWARE_SECTOR_SIZE]); },
                {},
                {
                    strncpy(error_msg, "Flash firmware area erase failed!", error_msg_len);
                    return secfalse;
                }
            );
            ...
            // write
            for ( size_t sector_offset = 0; sector_offset < FLASH_FIRMWARE_SECTOR_SIZE; sector_offset += 32 ) {
                // write with retry, max 10 retry allowed
                EXEC_RETRY(
                    10,
                    sectrue,
            ...
            EXEC_RETRY(
                ...
                {},
                {
                    return emmc_fs_file_write(
                        "0:/data/fw_p2.bin",
                        (processed_size - fw_internal_size),
 ...
 * @brief 验证固件头和内容
 *
 * @param vhdr vendor header
 * @param hdr image header
 * @param vhdr_valid vendor header 验证结果
 * @param hdr_valid image header 验证结果
 * @param code_valid code 验证结果
 * @param error_msg
 * @param error_msg_len
...
secbool verify_firmware(
    ...
    // optional arguments wipe default
    if ( vhdr != NULL )
        memset(vhdr, 0xff, sizeof(vendor_header));
    if ( hdr != NULL )
        memset(hdr, 0xff, sizeof(image_header));
    if ( vhdr_valid != NULL )
        *vhdr_valid = secfalse;
    ...
    // verify vhdr
    // 验证 vendor header
    ExecuteCheck_ADV(load_vendor_header((const uint8_t*)FIRMWARE_START, FW_KEY_M, FW_KEY_N, FW_KEYS, &_vhdr), sectrue, {
        if ( error_msg != NULL )
            strncpy(error_msg, "Firmware vendor header invalid!", error_msg_len);
        return secfalse;
    });

    _vhdr_valid = sectrue;
    if ( vhdr != NULL )
        memcpy(vhdr, &_vhdr, sizeof(vendor_header));
    if ( vhdr_valid != NULL )
        *vhdr_valid = _vhdr_valid;

    // verify hdr
    // 验证 image header
    ExecuteCheck_ADV(
        load_image_header(
            (const uint8_t*)(FIRMWARE_START + _vhdr.hdrlen),
    ...
    ExecuteCheck_ADV(
        check_image_contents_ADV(
            &_vhdr,
            &_hdr,
            (const uint8_t*)FIRMWARE_START + _vhdr.hdrlen + _hdr.hdrlen,
            0,
    ...
    int external_size = _hdr.codelen - (fw_internal_size - (_vhdr.hdrlen + _hdr.hdrlen));
    ...
    if ( external_size > 0 ) {
        ...
        if ( path_info.path_exist ) {
            ...
            ExecuteCheck_ADV(
                emmc_fs_file_read(
                    "0:data/fw_p2.bin", 0, (uint32_t*)FMC_SDRAM_FIRMWARE_P2_ADDRESS, external_size, &processed_len
                ),
                true,
                {
                    if ( error_msg != NULL )
                        strncpy(error_msg, "Firmware code invalid! (P2_EMMC_2)", error_msg_len);
                    return secfalse;
                }
            );
        } else if ( get_hw_ver() < HW_VER_3P0A ) {
            memcpy((uint8_t*)FMC_SDRAM_FIRMWARE_P2_ADDRESS, (const uint8_t*)0x90000000, external_size);
        }
        ...
        ExecuteCheck_ADV(
            check_image_contents_ADV(
                &_vhdr,
                &_hdr,
                (const uint8_t*)FMC_SDRAM_FIRMWARE_P2_ADDRESS,
                (fw_internal_size - (_vhdr.hdrlen + _hdr.hdrlen)),
                external_size,
                true
            ),
    ...
    return (((_vhdr_valid == sectrue) && (_hdr_valid == sectrue) && (_code_valid == sectrue))) ? sectrue : secfalse;
}
