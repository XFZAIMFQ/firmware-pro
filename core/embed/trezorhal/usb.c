...
#define USB_WINUSB_EXTRA_STRING \
  'M', 0x00, 'S', 0x00, 'F', 0x00, 'T', 0x00, '1', 0x00, '0', 0x00, '0', 0x00, USB_WINUSB_VENDOR_CODE, 0x00 // MSFT100!
...
void usb_init(const usb_dev_info_t* dev_info) {
    ...
    usb_config_desc->wTotalLength = sizeof(usb_config_descriptor_t); // will be updated later via
                                                                     // usb_desc_add_iface()
    ...
    usb_next_iface_desc = (usb_interface_descriptor_t*)(usb_config_buf + usb_config_desc->wTotalLength);
        ...
        sectrue * (USBD_OK == USBD_Init(&usb_dev_handle, (USBD_DescriptorsTypeDef*)&usb_descriptors, USB_PHY_ID)), NULL
    );
...
static uint8_t usb_class_setup(USBD_HandleTypeDef* dev, USBD_SetupReqTypedef* req) {
    ...
    if ( (req->bmRequest & USB_REQ_TYPE_MASK) == USB_REQ_TYPE_VENDOR ) {
        if ( (req->bmRequest & USB_REQ_RECIPIENT_MASK) == USB_REQ_RECIPIENT_DEVICE ) {
            ...
            } else if ( sectrue == usb21_enabled && req->bRequest == USB_WINUSB_VENDOR_CODE ) {
