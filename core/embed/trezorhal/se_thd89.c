...
secbool se_fp_reset_se(void) {
    return se_reset_se_ex(THD89_FINGER_ADDRESS);
}

static void cal_mac(uint8_t* session_key, uint8_t* data, uint32_t len, uint8_t* mac) {
 ...
 * @param recv          应答
 * @param recv_len      应答长度
 * @return secbool      成功返回sectrue，失败返回secfalse
 */
static secbool se_transmit_mac_ex(
    uint8_t addr,
    uint8_t* session_key,
    uint8_t ins,
    uint8_t p1,
...
secbool se_transmit_mac(
    ...
    thd89_irq_nest++;
    secbool result =
        se_transmit_mac_ex(THD89_MASTER_ADDRESS, se_session_key, ins, p1, p2, data, data_len, recv, recv_len);
    thd89_irq_nest--;
    if ( thd89_irq_nest == 0 ) {
        enable_irq(irq);
    }
...
secbool se_fp_transmit_mac(
    ...
    thd89_irq_nest++;
    secbool result =
        se_transmit_mac_ex(THD89_FINGER_ADDRESS, se_fp_session_key, ins, p1, p2, data, data_len, recv, recv_len);
    thd89_irq_nest--;
    if ( thd89_irq_nest == 0 ) {
        enable_irq(irq);
    }
...
secbool se_sync_session_key_ex_old(uint8_t addr, uint8_t* session_key) {
    ...
    ensure(flash_otp_read(FLASH_OTP_BLOCK_THD89_SESSION_KEY, 0, default_key, 16), NULL);
    ...
    if ( !thd89_transmit_ex(addr, sync_cmd, sizeof(sync_cmd), data_buf, &recv_len) ) {
...
static secbool se_sync_session_key_ex(uint8_t addr, uint8_t* session_key) {
    ...
    if ( !thd89_transmit_ex(addr, sync_cmd, sizeof(sync_cmd), signature, &recv_len) ) {
        memset(session_key, 0x00, SESSION_KEYLEN);
        return secfalse;
    }
    if ( recv_len != 64 ) {
...
secbool se_derive_keys(
    ...
    if ( !se_transmit_mac(SE_INS_DERIVE, 0x00, 0x00, APDU_DATA, len, resp, &resp_len) ) {
        return secfalse;
    }
...
secbool se_derive_xmr_key(
    ...
    if ( !se_transmit_mac(SE_INS_DERIVE, 0x00, 0x01, APDU_DATA, len, resp, &resp_len) ) {
        return secfalse;
    }
...
secbool se_derive_xmr_private_key(const uint8_t* pubkey, const uint32_t index, uint8_t* prikey) {
    ...
    if ( !se_transmit_mac(SE_INS_DERIVE, 0x00, 0x02, data, sizeof(data), resp, &resp_len) ) {
        return secfalse;
    }
...
secbool se_xmr_get_tx_key(const uint8_t* rand, const uint8_t* hash, uint8_t* out) {
    ...
    if ( !se_transmit_mac(SE_INS_DERIVE, 0x00, 0x03, data, sizeof(data), resp, &resp_len) ) {
        return secfalse;
    }
    memcpy(out, resp, 32);
...
static int _se_get_ver_info(uint8_t addr, uint8_t cmd, uint8_t* out, uint16_t in_len) {
...
char* se04_get_boot_build_id(void) {
    ...
    int len = se_get_boot_build_id(THD89_FINGER_ADDRESS, build_id, sizeof(build_id));
    if ( len == 0 ) {
        return NULL;
    }
    return build_id;
...
secbool se_set_session_key(const uint8_t* session_key) {
    ensure(se_set_session_key_ex(THD89_MASTER_ADDRESS, session_key), "se set session key failed");
    ensure(se_set_session_key_ex(THD89_FINGER_ADDRESS, session_key), "se fp set session key failed");
    return sectrue;
}
...
static secbool se_hasPin_ex(uint8_t addr, uint8_t* session_key) {
    ...
    ensure(se_transmit_mac_ex(addr, session_key, SE_INS_PIN, 0x00, 0x00, NULL, 0, &hasPin, &len), "get pin failed");
...
static secbool se_setPin_ex(uint8_t addr, uint8_t* session_key, const char* pin) {
    ...
    if ( !se_transmit_mac_ex(addr, session_key, SE_INS_PIN, 0x00, 0x01, pin_buf, pin_buf[0] + 1, NULL, NULL) ) {
...
secbool se_setPin(const char* pin) {
    ...
    if ( result == sectrue ) {
        secbool fp_result = se_setPin_ex(THD89_FINGER_ADDRESS, se_fp_session_key, pin);
...
static secbool se_verifyPin_ex(uint8_t addr, uint8_t* session_key, const char* pin, pin_type_t pin_type) {
    ...
    if ( !se_transmit_mac_ex(addr, session_key, SE_INS_PIN, 0x00, 0x03, pin_buf, data_len, resp, &resp_len) ) {
        memset(pin_buf, 0, sizeof(pin_buf));
        if ( 0x6f80 == thd89_last_error() ) {
            error_reset("You have entered the", "wipe code. All private", "data has been erased.", NULL);
        }
    ...
    if ( pin_type == PIN_TYPE_PASSPHRASE_PIN && pin_result_type != PASSPHRASE_PIN_ENTERED ) {
        return secfalse;
    }
...
static secbool se_fp_verifyPin(const char* pin) {
    return se_verifyPin_ex(THD89_FINGER_ADDRESS, se_fp_session_key, pin, PIN_TYPE_USER);
}
...
secbool se_verifyPin(const char* pin, pin_type_t pin_type) {
    secbool result = se_verifyPin_ex(THD89_MASTER_ADDRESS, se_session_key, pin, pin_type);
    if ( result == sectrue ) {
        if ( pin_type != PIN_TYPE_PASSPHRASE_PIN_CHECK ) {
            secbool fp_result = se_verifyPin_ex(THD89_FINGER_ADDRESS, se_fp_session_key, pin, pin_type);
            if ( fp_result == sectrue ) {
                return sectrue;
            }
            // else {
...
static secbool se_changePin_ex(uint8_t addr, uint8_t* session_key, const char* oldpin, const char* newpin) {
    ...
    if ( !se_transmit_mac_ex(
             addr, session_key, SE_INS_PIN, 0x00, 0x02, pin_buff, strlen(oldpin) + strlen(newpin) + 2, NULL, NULL
         ) ) {
...
secbool se_changePin(const char* oldpin, const char* newpin) {
    ...
    secbool result = se_changePin_ex(THD89_MASTER_ADDRESS, se_session_key, oldpin, newpin);
    if ( result == sectrue ) {
        secbool fp_result = se_changePin_ex(THD89_FINGER_ADDRESS, se_fp_session_key, oldpin, newpin);
        if ( fp_result == sectrue ) {
            return sectrue;
        } else {
            if ( se_fp_hasPin() ) {
...
static secbool se_getRetryTimes_ex(uint8_t addr, uint8_t* session_key, uint8_t* ptimes) {
    uint8_t remain;
    uint16_t recv_len = 1;

    if ( !se_transmit_mac_ex(addr, session_key, SE_INS_PIN, 0x00, 0x05, NULL, 0, &remain, &recv_len) ) {
        return secfalse;
    }
    *ptimes = remain;
...
secbool se_getRetryTimes(uint8_t* ptimes) {
    ...
    ensure(se_getRetryTimes_ex(THD89_MASTER_ADDRESS, se_session_key, &retry_cnts), "get master retry times failed");
    ensure(se_getRetryTimes_ex(THD89_FINGER_ADDRESS, se_fp_session_key, &fp_retry_cnts), "get fp retry times failed");
...
static secbool se_clearSecsta_ex(uint8_t addr, uint8_t* session_key) {
    uint16_t recv_len = 0;
    if ( !se_transmit_mac_ex(addr, session_key, SE_INS_PIN, 0x00, 0x06, NULL, 0, NULL, &recv_len) ) {
        return secfalse;
    }
...
static secbool se_set_pin_passphrase_ex(
    uint8_t addr,
    uint8_t* session_key,
    const char* pin,
    ...
    if ( !se_transmit_mac_ex(addr, session_key, SE_INS_PIN, 0x00, 0x09, buf, offset, resp, &resp_len) ) {
...
secbool se_set_pin_passphrase(const char* pin, const char* passphrase_pin, const char* passphrase, bool* override) {
    secbool result =
        se_set_pin_passphrase_ex(THD89_MASTER_ADDRESS, se_session_key, pin, passphrase_pin, passphrase, override);
    if ( result == sectrue ) {
        secbool fp_result = se_set_pin_passphrase_ex(
            THD89_FINGER_ADDRESS, se_fp_session_key, pin, passphrase_pin, passphrase, override
        );
        if ( fp_result == sectrue ) {
            return sectrue;
        }
...
static secbool
se_delete_pin_passphrase_ex(uint8_t addr, uint8_t* session_key, const char* passphrase_pin, bool* current) {
    if ( strlen(passphrase_pin) < 6 || strlen(passphrase_pin) > PIN_MAX_LENGTH ) {
        return secfalse;
    }
    ...
    if ( !se_transmit_mac_ex(
             addr, session_key, SE_INS_PIN, 0x00, 0x0A, buf, strlen(passphrase_pin) + 1, resp, &resp_len
         ) ) {
        return secfalse;
    }
    if ( resp[0] == PIN_SUCCESS ) {
...
secbool se_delete_pin_passphrase(const char* passphrase_pin, bool* current) {
    secbool result = se_delete_pin_passphrase_ex(THD89_MASTER_ADDRESS, se_session_key, passphrase_pin, current);
    if ( result == sectrue ) {
        secbool fp_result =
            se_delete_pin_passphrase_ex(THD89_FINGER_ADDRESS, se_fp_session_key, passphrase_pin, current);
        if ( fp_result == sectrue ) {
            return sectrue;
        }
        return secfalse;
...
secbool se_check_passphrase_btc_test_address(const char* address) {
    ...
    if ( !se_transmit_mac(SE_INS_PIN, 0x00, 0x0B, buf, strlen(address) + 1, resp, &resp_len) ) {
        return secfalse;
    }
    if ( resp[0] == 0x55 ) {
        return sectrue;
...
secbool se_change_pin_passphrase_ex(uint8_t addr, uint8_t* session_key, const char* old_pin, const char* new_pin) {
    uint8_t buf[128];
    uint8_t resp[1];
    uint16_t resp_len = 1;

    if ( strlen(old_pin) < 6 || strlen(old_pin) > PIN_MAX_LENGTH || strlen(new_pin) < 6 ||
         strlen(new_pin) > PIN_MAX_LENGTH ) {
        return secfalse;
    }
    ...
    if ( !se_transmit_mac_ex(
             addr, session_key, SE_INS_PIN, 0x00, 0x0E, buf, 1 + strlen(old_pin) + 1 + strlen(new_pin), resp, &resp_len
         ) ) {
        return secfalse;
    }
    if ( resp[0] == PIN_SUCCESS ) {
...
secbool se_change_pin_passphrase(const char* old_pin, const char* new_pin) {
    secbool result = se_change_pin_passphrase_ex(THD89_MASTER_ADDRESS, se_session_key, old_pin, new_pin);
    if ( result == sectrue ) {
        secbool fp_result = se_change_pin_passphrase_ex(THD89_FINGER_ADDRESS, se_fp_session_key, old_pin, new_pin);
        if ( fp_result == sectrue ) {
            return sectrue;
        }
        return secfalse;
...
pin_result_t se_get_pin_result_type(void) {
    return pin_result_type;
}
pin_result_t se_get_pin_passphrase_ret(void) {
    return pin_passphrase_ret;
}

secbool se_clearSecsta(void) {
    ensure(se_clearSecsta_ex(THD89_MASTER_ADDRESS, se_session_key), "clear master secsta failed");
...
static secbool se_getSecsta_ex(uint8_t addr, uint8_t* session_key) {
    ...
    if ( !se_transmit_mac_ex(addr, session_key, SE_INS_PIN, 0x00, 0x04, NULL, 0, &cur_secsta, &recv_len) ) {
        return secfalse;
    }
...
secbool se_getSecsta(void) {
    ...
    if ( result == sectrue ) {
        secbool fp_result = se_getSecsta_ex(THD89_FINGER_ADDRESS, se_fp_session_key);
        if ( fp_result == sectrue ) {
            return sectrue;
        }
    }
...
secbool se_set_mnemonic(const char* mnemonic, uint16_t len) {
    return se_transmit_mac(0xE2, 0x00, 0x00, (uint8_t*)mnemonic, len, NULL, NULL);
}
...
secbool se_sessionStart(uint8_t* session_id_bytes) {
    ...
    if ( !se_transmit_mac(SE_INS_SESSION, 0x00, 0x00, NULL, 0, session_id_bytes, &recv_len) ) {
        return secfalse;
    }
...
secbool se_sessionOpen(uint8_t* session_id_bytes) {
    uint16_t recv_len = 32;
    if ( !se_transmit_mac(SE_INS_SESSION, 0x00, 0x01, session_id_bytes, 32, session_id_bytes, &recv_len) ) {
        return secfalse;
    }
    return sectrue;
}
...
secbool se_set_public_region(const uint16_t offset, const void* val_dest, uint16_t len) {
    uint8_t cmd[4] = {0};
    ...
    if ( !se_transmit_mac(SE_INS_WRITE_DATA, 0x00, 0x00, APDU_DATA, 4 + len, NULL, NULL) ) {
        return secfalse;
    }
    return sectrue;
}
...
secbool se_get_public_region(uint16_t offset, void* val_dest, uint16_t len) {
    ...
    cmd[2] = (len >> 8) & 0xFF;
    cmd[3] = len & 0xFF;
    if ( !se_transmit_mac(SE_INS_READ_DATA, 0x00, 0x00, cmd, sizeof(cmd), val_dest, &recv_len) ) {
        return secfalse;
    }
    return sectrue;
}
...
secbool se_set_private_region(uint16_t offset, const void* val_dest, uint16_t len) {
    uint8_t cmd[4] = {0};
    if ( offset + len > PRIVATE_REGION_SIZE )
        return secfalse;
    cmd[0] = (offset >> 8) & 0xFF;
    cmd[1] = offset & 0xFF;
    cmd[2] = (len >> 8) & 0xFF;
    ...
    memcpy(APDU_DATA, cmd, 4);
    memcpy(APDU_DATA + 4, (uint8_t*)val_dest, len);
    if ( !se_transmit_mac(SE_INS_WRITE_DATA, 0x00, 0x01, APDU_DATA, 4 + len, NULL, NULL) ) {
        return secfalse;
    }
    return sectrue;
}
...
secbool se_get_private_region(uint16_t offset, void* val_dest, uint16_t len) {
    ...
    cmd[2] = (len >> 8) & 0xFF;
    cmd[3] = len & 0xFF;
    if ( !se_transmit_mac(SE_INS_READ_DATA, 0x00, 0x01, cmd, sizeof(cmd), val_dest, &recv_len) ) {
        return secfalse;
    }
    return sectrue;
}
...
secbool se_containsMnemonic(const char* mnemonic) {
    ...
    if ( !se_transmit_mac(0xE2, 0x00, 0x01, (uint8_t*)mnemonic, strlen(mnemonic), &verify, &len) ) {
        return secfalse;
    }
...
secbool se_set_needs_backup(bool needs_backup) {
    if ( !se_transmit_mac(0xE2, 0x00, 0x03, (uint8_t*)&needs_backup, 1, NULL, NULL) ) {
        return secfalse;
    }
...
secbool se_changeWipeCode(const char* pin, const char* wipe_code) {
    ...
    if ( !se_transmit_mac(SE_INS_PIN, 0x00, 0x08, pin_buff, strlen(pin) + strlen(wipe_code) + 2, NULL, NULL) ) {
        memset(pin_buff, 0, sizeof(pin_buff));
        return secfalse;
    }
    memset(pin_buff, 0, sizeof(pin_buff));
...
static int _se_ed25519_send_msg(uint8_t ins, uint8_t type, const uint8_t* msg, uint16_t msg_len) {
...
static int se_ed25519_sign_digest(const uint8_t* msg, uint16_t msg_len, uint8_t type, uint8_t* sig) {
...
int se_ed25519_sign(const uint8_t* msg, uint16_t msg_len, uint8_t* sig) {
    ...
    if ( msg_len > SE_DATA_MAX_LEN ) {
        ...
        if ( !se_transmit_mac(SE_INS_SIGN, 0x00, 0x02, (uint8_t*)msg, msg_len, resp, &resp_len) ) {
            return -1;
        }
    }
...
int se_ed25519_sign_ext(const uint8_t* msg, uint16_t msg_len, uint8_t* sig) {
    ...
    if ( msg_len > SE_DATA_MAX_LEN ) {
        ...
        if ( !se_transmit_mac(SE_INS_SIGN, 0x00, 0x03, (uint8_t*)msg, msg_len, resp, &resp_len) ) {
            return -1;
        }
    }
    memcpy(sig, resp, resp_len);
...
int se_ed25519_sign_keccak(const uint8_t* msg, uint16_t msg_len, uint8_t* sig) {
    ...
    if ( msg_len > SE_DATA_MAX_LEN ) {
        ...
        if ( !se_transmit_mac(SE_INS_SIGN, 0x00, 0x04, (uint8_t*)msg, msg_len, resp, &resp_len) ) {
            return -1;
        }
    }
    memcpy(sig, resp, resp_len);
...
secbool se_session_is_open() {
    ...
    if ( !se_transmit_mac(SE_INS_SESSION, 0x00, 0x07, NULL, 0, &state, &recv_len) ) {
        return secfalse;
    }
...
secbool session_generate_master_seed(const char* passphrase, uint8_t* percent) {
    if ( !se_transmit_mac(SE_INS_SESSION, 0x00, 0x05, (uint8_t*)passphrase, strlen(passphrase), NULL, NULL) ) {
        if ( thd89_last_error() == 0x6c00 ) {
            *percent = 0;
            return sectrue;
        }
...
secbool session_generate_cardano_seed(const char* passphrase, uint8_t* percent) {
    if ( !se_transmit_mac(SE_INS_SESSION, 0x00, 0x06, (uint8_t*)passphrase, strlen(passphrase), NULL, NULL) ) {
        if ( thd89_last_error() == 0x6c00 ) {
            *percent = 0;
            return sectrue;
        }
...
secbool se_node_sign_digest(const uint8_t* hash, uint8_t* sig, uint8_t* by) {
    ...
    if ( !se_transmit_mac(SE_INS_SIGN, 0x00, 0x00, (uint8_t*)hash, 32, resp, &resp_len) ) {
        return secfalse;
    }
...
int se_ecdsa_ecdh(const uint8_t* publickey, uint8_t* sessionkey) {
    ...
    if ( !se_transmit_mac(SE_INS_ECDH, 0x00, 0x00, (uint8_t*)publickey, 64, resp, &resp_len) ) {
        return -1;
    }
...
int se_curve25519_ecdh(const uint8_t* publickey, uint8_t* sessionkey) {
    ...
    if ( !se_transmit_mac(SE_INS_ECDH, 0x00, 0x01, (uint8_t*)publickey, 32, resp, &resp_len) ) {
        return -1;
    }
    memcpy(sessionkey, resp, resp_len);
    return 0;
...
int se_lite_card_ecdh(const uint8_t* publickey, uint8_t* sessionkey) {
    ...
    if ( !se_transmit_mac(SE_INS_ECDH, 0x00, 0x02, (uint8_t*)publickey, 64, resp, &resp_len) ) {
        return -1;
    }
    memcpy(sessionkey, resp, resp_len);
    return 0;
...
int se_get_shared_key(const char* curve, const uint8_t* peer_public_key, uint8_t* session_key) {
    if ( strcmp(curve, NIST256P1_NAME) == 0 || strcmp(curve, SECP256K1_NAME) == 0 ) {
...
int se_bip340_sign_digest(const uint8_t* digest, uint8_t sig[64]) {
    uint8_t resp[64];
    uint16_t resp_len = sizeof(resp);
    if ( !se_transmit_mac(SE_INS_SIGN, 0x00, 0x07, (uint8_t*)digest, 32, resp, &resp_len) ) {
        return -1;
    }
    if ( resp_len != 64 )
        return -1;
...
int se_bch_schnorr_sign_digest(const uint8_t* digest, uint8_t sig[64]) {
    uint8_t resp[64];
    uint16_t resp_len = sizeof(resp);
    if ( !se_transmit_mac(SE_INS_SIGN, 0x00, 0x09, (uint8_t*)digest, 32, resp, &resp_len) ) {
        return -1;
    }
    if ( resp_len != 64 )
        return -1;
...
int se_aes256_encrypt(
    ...
    if ( !se_transmit_mac(SE_INS_AES, 0x00, 0x00, APDU_DATA, len, out, &resp_len) ) {
        return -1;
    }
    return 0;
}
...
int se_aes256_decrypt(
    ...
    if ( !se_transmit_mac(SE_INS_AES, 0x00, 0x01, APDU_DATA, len, out, &resp_len) ) {
        return -1;
    }
    return 0;
}
...
int se_nem_aes256_encrypt(
    ...
    if ( !se_transmit_mac(SE_INS_AES, 0x00, 0x02, APDU_DATA, len, out, &resp_len) ) {
        return -1;
    }
    return 0;
}
...
int se_nem_aes256_decrypt(
    ...
    if ( !se_transmit_mac(SE_INS_AES, 0x00, 0x03, APDU_DATA, len, out, &resp_len) ) {
        return -1;
    }
    return 0;
}
...
secbool
se_authorization_set(const uint32_t authorization_type, const uint8_t* authorization, uint32_t authorization_len) {
    uint8_t data[128];
    ...
    if ( !se_transmit_mac(SE_INS_COINJOIN, 0x00, 0x00, data, authorization_len + 4, NULL, NULL) ) {
        return secfalse;
    }
    return sectrue;
}
...
secbool se_authorization_get_type(uint32_t* authorization_type) {
    uint32_t type = 0;
    uint16_t resp_len = 4;
    if ( !se_transmit_mac(SE_INS_COINJOIN, 0x00, 0x01, NULL, 0, (uint8_t*)&type, &resp_len) ) {
        return secfalse;
    }
...
secbool se_authorization_get_data(uint8_t* authorization_data, uint32_t* authorization_len) {
    ...
    if ( !se_transmit_mac(SE_INS_COINJOIN, 0x00, 0x02, NULL, 0, authorization_data, &resp_len) ) {
        return secfalse;
    }
...
secbool se_fingerprint_state(void) {
    ...
    if ( !se_transmit_mac(SE_INS_FINGERPRINT, 0x00, 0x00, NULL, 0, &state, &recv_len) ) {
        return secfalse;
    }
    // 0x55 is verified pin 0x00 is not verified pin
    return sectrue * (state == 0x55);
...
secbool se_fp_write(uint32_t offset, const void* val_dest, uint32_t len, uint8_t index, uint8_t total) {
    ...
    while ( len ) {
        ...
        memcpy(APDU_DATA + (size_4 ? 8 : 4), (uint8_t*)val_dest + packet_offset, packet_len);
        if ( !se_fp_transmit_mac(
                 SE_INS_WRITE_DATA, 0x00, 0x02, APDU_DATA, (size_4 ? 8 : 4) + packet_len, NULL, NULL
             ) ) {
            return secfalse;
        }
...
secbool se_fp_read(uint32_t offset, void* val_dest, uint32_t len, uint8_t index, uint8_t total) {
    ...
    while ( len ) {
        ...
        if ( !se_fp_transmit_mac(
                 SE_INS_READ_DATA, 0x00, 0x02, cmd, (size_4 ? 8 : 4), (uint8_t*)val_dest + packet_offset, &packet_len
             ) ) {
            return secfalse;
        }
...
secbool se_get_fido2_data(uint16_t offset, uint8_t* dest, uint16_t len) {
    ...
    cmd[2] = (len >> 8) & 0xFF;
    cmd[3] = len & 0xFF;
    if ( !se_transmit_mac(SE_INS_READ_DATA, 0x00, 0x03, cmd, sizeof(cmd), dest, &recv_len) ) {
        return secfalse;
    }
    return sectrue;
}
...
secbool se_set_fido2_data(uint16_t offset, const uint8_t* src, uint16_t len) {
    ...
    if ( !se_transmit_mac(SE_INS_WRITE_DATA, 0x00, 0x03, APDU_DATA, 4 + len, NULL, NULL) ) {
        return secfalse;
    }
    return sectrue;
}
...
secbool se_get_fido2_resident_credentials(uint32_t index, uint8_t* dest, uint16_t* dst_len) {
    ...
    if ( memcmp(cred_id->credential_id_flag, FIDO2_RESIDENT_CREDENTIALS_FLAGS, 4) != 0 ) {
        return secfalse;
    }
    if ( *dst_len < cred_id->credential_length ) {
        return secfalse;
    }
    if ( !se_get_fido2_data(index * FIDO2_RESIDENT_CREDENTIALS_SIZE + 6, buffer + 6, cred_id->credential_length) ) {
        return secfalse;
    }
...
secbool se_set_fido2_resident_credentials(uint32_t index, const uint8_t* src, uint16_t len) {
    if ( index >= FIDO2_RESIDENT_CREDENTIALS_COUNT )
        return secfalse;
    if ( len > (FIDO2_RESIDENT_CREDENTIALS_SIZE - 6) )
        return secfalse;
    ...
    return se_set_fido2_data(index * FIDO2_RESIDENT_CREDENTIALS_SIZE, (uint8_t*)&cred_id, 6 + len);
}

secbool se_delete_fido2_resident_credentials(uint32_t index) {
    uint8_t buffer[FIDO2_RESIDENT_CREDENTIALS_HEADER_LEN] = {0xff};
    return se_set_fido2_data(index * FIDO2_RESIDENT_CREDENTIALS_SIZE, buffer, FIDO2_RESIDENT_CREDENTIALS_HEADER_LEN);
}
