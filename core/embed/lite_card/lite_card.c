...
static bool lite_card_send_device_certificate(uint8_t* cert, uint8_t cert_len) {
    ...
    if ( !nfc_send_recv(apdu_send_device_cert, cert_len + 5, resp, &resp_len, sw1sw2) ) {
        return false;
    }
    if ( sw1sw2[0] != 0x90 || sw1sw2[1] != 0x00 ) {
        return false;
...
static bool lite_card_mutual_authentication(scp11_context* scp11_ctx) {
    ...
    scp11_get_mutual_auth_data(apdu_mutual_auth + 5, &apdu_mutual_auth[4], *scp11_ctx);
    if ( !nfc_send_recv(
             apdu_mutual_auth,
             apdu_mutual_auth[4] + 5,
             scp11_ctx->response_msg.data,
             &scp11_ctx->response_msg.data_len,
             sw1sw2
         ) ) {
        return false;
    }
...
bool lite_card_send_safeapdu(
    ...
    scp03_encrypt(scp11_ctx.session_key.s_enc, scp03_ctx.icv, data_buffer + 5, apdu_len - 5, enc_data, &enc_data_len);
    ...
    memcpy(mac_data, scp03_ctx.mac_chain_value, sizeof(scp03_ctx.mac_chain_value));
    memcpy(mac_data + 16, data_buffer, 5);
    memcpy(mac_data + 21, enc_data, enc_data_len);

    AES128_CMAC(scp11_ctx.session_key.s_mac, mac_data, enc_data_len + 5 + 16, mac);
    ...
    if ( !nfc_send_recv(data_buffer, enc_data_len + 5 + 8, data_buffer, &buffer_len, sw1sw2) ) {
        return false;
    }
    ...
    if ( !scp03_decrypt(scp11_ctx.session_key.s_enc, scp03_ctx.icv, data_buffer, buffer_len, enc_data, &buffer_len) ) {
        return false;
    }
...
bool lite_card_open_secure_channel(void) {
    ...
    if ( !lite_card_get_sd_certificate(scp11_ctx.sd_cert.raw, &scp11_ctx.sd_cert.raw_len) ) {
        return false;
    }
    if ( !scp11_certificate_parse_and_verify(scp11_ctx.sd_cert.raw, scp11_ctx.sd_cert.raw_len, &scp11_ctx.sd_cert) ) {
        return false;
    }
    if ( !lite_card_send_device_certificate(scp11_ctx.oce_cert.raw, scp11_ctx.oce_cert.raw_len) ) {
        return false;
    }
...
bool lite_card_apdu(
    ...
    if ( safe ) {
        ...
        return lite_card_send_safeapdu(apdu, apdu_len, response, response_len, sw1sw2);
    }
...
bool lite_card_safe_apdu_test(void) {
    ...
    uint8_t select_old_aid[] = {0x00, 0xa4, 0x04, 0x00, 0x08, 0xD1, 0x56, 0x00, 0x01, 0x32, 0x83, 0x40, 0x01};
    uint8_t reset_card[] = {0x80, 0xcb, 0x80, 0x00, 0x05, 0xdf, 0xfe, 0x02, 0x82, 0x05};
    uint8_t check_pin[] = {0x80, 0xcb, 0x80, 0x00, 0x05, 0xdf, 0xff, 0x02, 0x81, 0x05};
    uint8_t set_pin[] = {
        0x80,
        0xcb,
        0x80,
        ...
        0x35,
        0x36};
    uint8_t verify_pin[] = {0x80, 0x20, 0x00, 0x00, 0x07, 0x06, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36};
    uint8_t resp[256];
    uint16_t resp_len = sizeof(resp);
    uint8_t sw1sw2[2] = {0};
    ...
    if ( !lite_card_apdu(reset_card, sizeof(reset_card), resp, &resp_len, sw1sw2, true) ) {
        return false;
    }
    resp_len = sizeof(resp);
    if ( !lite_card_apdu(check_pin, sizeof(check_pin), resp, &resp_len, sw1sw2, true) ) {
        return false;
    }

    resp_len = sizeof(resp);
    if ( !lite_card_apdu(set_pin, sizeof(set_pin), resp, &resp_len, sw1sw2, true) ) {
        return false;
    }
    ...
    lite_card_apdu(select_old_aid, sizeof(select_old_aid), resp, &resp_len, sw1sw2, false);

    resp_len = sizeof(resp);
    if ( !lite_card_apdu(verify_pin, sizeof(verify_pin), resp, &resp_len, sw1sw2, true) ) {
        return false;
    }
    return true;
}
