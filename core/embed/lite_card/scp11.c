...
const uint8_t shared_info_buffer[] = {
    GPC_TLV_SHAREDINFO_SCP_ID_PARAM,
    0x02,
    0x11,
    0x07,
...
const uint8_t shared_info_data_buffer[] = {0x3c, 0x88, 0x10, 0x08, 0x70, 0x72, 0x6F, 0x2D, 0x6C, 0x69, 0x74, 0x65};
...
static bool scp11_get_signature(uint8_t* input, uint8_t inpue_len, uint8_t* signature) {
    uint8_t* p = input;
    uint8_t* end = input + inpue_len;
    uint16_t value_len = 0;
    uint16_t offset = 0;
...
bool scp11_certificate_parse_and_verify(uint8_t* cert_raw, uint16_t cert_len, scp11_certificate* cert) {
    // Parse certificate
    uint16_t tag;
    ...
    while ( p < end ) {
        ...
        if ( is_nested ) {
            ...
            case GPC_TLV_SCP11CRT_PUBKEY:
                ...
                sha256_Update(&ctx, p - tag_len - len_len, tag_len + len_len + value_len);
                p += value_len;
                break;
    ...
    if ( !scp11_get_signature(
             cert->lv_GPC_TLV_SCP11CRT_SIGNATURE.value, cert->lv_GPC_TLV_SCP11CRT_SIGNATURE.len, signature
         ) ) {
        return false;
    }

    return ecdsa_verify_digest(&nist256p1, sd_cert_verify_pubkey, signature, digest) == 0;
}

bool scp11_get_mutual_auth_data(uint8_t* data, uint8_t* data_len, scp11_context scp11_ctx) {
    uint8_t* p = data;

    uint8_t len = sizeof(shared_info_buffer) + 2 + 68; // 数据长度
...
bool scp11_open_secure_channel(scp11_context* scp11_ctx) {
    ...
    scp11_get_pubkey(
        scp11_ctx->sd_cert.lv_GPC_TLV_SCP11CRT_PUBKEY.value,
    ...
    if ( 0 !=
         ecdh_multiply(
             &nist256p1, scp11_ctx->mutual_auth.oce_temp_private_key, scp11_ctx->mutual_auth.sd_public_key, shses_key
         ) ) {
        return false;
    }
    ...
    sha256_Update(
        &ctx, scp11_ctx->sd_cert.lv_GPC_TLV_SCP11CRT_CAKLOCID.value, scp11_ctx->sd_cert.lv_GPC_TLV_SCP11CRT_CAKLOCID.len
    );
    sha256_Final(&ctx, digest);
...
void scp11_init(scp11_context* scp11_ctx) {
    ...
    memcpy(scp11_ctx->oce_cert.raw, device_certificate, sizeof(device_certificate) - 1);
    ...
    if ( !scp11_certificate_parse_and_verify(
             scp11_ctx->oce_cert.raw, scp11_ctx->oce_cert.raw_len, &scp11_ctx->oce_cert
         ) ) {
        return;
    }

    scp11_get_pubkey(
        scp11_ctx->oce_cert.lv_GPC_TLV_SCP11CRT_PUBKEY.value,
        scp11_ctx->oce_cert.lv_GPC_TLV_SCP11CRT_PUBKEY.len,
        scp11_ctx->mutual_auth.oce_public_key
    );

    random_buffer(scp11_ctx->mutual_auth.oce_temp_private_key, sizeof(scp11_ctx->mutual_auth.oce_temp_private_key));

    ecdsa_get_public_key65(
        &nist256p1, scp11_ctx->mutual_auth.oce_temp_private_key, scp11_ctx->mutual_auth.oce_temp_public_key
    );
}
