...
static int dummy_dbg_printf(const char* fmt, ...) {
    return 0;
}

static DbgPrintf_t dbg_printf = dummy_dbg_printf;

inline void disp_set_dbg_printf(DbgPrintf_t func) {
    dbg_printf = func;
}
...
#define LTDC_FREQ                                                                             \
  (uint32_t                                                                                   \
  )(HSE_VALUE / 5 * (lcd_params.ltdc_pll.PLL3N + (lcd_params.ltdc_pll.PLL3FRACN / 8192.0F)) / \
...
#define FPS_TARGET                                                                               \
  (uint32_t                                                                                      \
  )((float)LTDC_FREQ / ((lcd_params.hres + lcd_params.hbp + lcd_params.hfp + lcd_params.hsync) * \
                        (lcd_params.vres + lcd_params.vbp + lcd_params.vfp + lcd_params.vsync)))
...
void lcd_pwm_init(void) {
    ...
    // we want 1 MHz
    TIM1_Handle.Init.Prescaler = SystemCoreClock / 1000000 / 4 - 1; // APB is fCPU/2(AHB)/2(APB)
    ...
    TIM_OC_InitStructure.Pulse = (LED_PWM_TIM_PERIOD / 2 - 1); // default 50% dutycycle
...
static HAL_StatusTypeDef ltdc_init(LTDC_HandleTypeDef* hltdc) {
    ...
    hltdc->Init.AccumulatedActiveW = lcd_params.hsync + lcd_params.hres + lcd_params.hbp - 1;
    hltdc->Init.TotalWidth = lcd_params.hsync + lcd_params.hres + lcd_params.hbp + lcd_params.hfp - 1;
    hltdc->Init.VerticalSync = lcd_params.vsync - 1;
    hltdc->Init.AccumulatedVBP = lcd_params.vsync + lcd_params.vbp - 1;
    hltdc->Init.AccumulatedActiveH = lcd_params.vsync + lcd_params.vres + lcd_params.vbp - 1;
    hltdc->Init.TotalHeigh = lcd_params.vsync + lcd_params.vres + lcd_params.vbp + lcd_params.vfp - 1;
...
static HAL_StatusTypeDef ltdc_layer_config(LTDC_HandleTypeDef* hltdc, uint32_t layer_index, LTDC_LAYERCONFIG* config) {
...
static HAL_StatusTypeDef dsi_host_init(DSI_HandleTypeDef* hdsi) {
    ...
    // video mode
    {
        ...
        VidCfg.HorizontalLine =
            ((lcd_params.hres + lcd_params.hsync + lcd_params.hbp + lcd_params.hfp) * DSI_FREQ) / LTDC_FREQ;
...
void fb_read_pixel(uint32_t x_pos, uint32_t y_pos, uint32_t* color) {
    if ( lcd_params.pixel_format_ltdc == LTDC_PIXEL_FORMAT_ARGB8888 ) {
        ...
        *color = *(uint32_t*)(lcd_params.fb_base + (lcd_params.bbp * (lcd_params.hres * y_pos + x_pos)));
    } else {
        /*LTDC_PIXEL_FORMAT_RGB565 */
        *color = *(uint16_t*)(lcd_params.fb_base + (lcd_params.bbp * (lcd_params.hres * y_pos + x_pos)));
    }
}

void fb_write_pixel(uint32_t x_pos, uint32_t y_pos, uint32_t color) {
    if ( x_pos < lcd_window.start_x || x_pos >= lcd_window.end_x || y_pos < lcd_window.start_y ||
         y_pos >= lcd_window.end_y ) {
        return;
    }
    if ( lcd_params.pixel_format_ltdc == LTDC_PIXEL_FORMAT_ARGB8888 ) {
        *(uint32_t*)(lcd_params.fb_base + (lcd_params.bbp * (lcd_params.hres * y_pos + x_pos))) = color;
    } else {
        /*LTDC_PIXEL_FORMAT_RGB565 */
        *(uint16_t*)(lcd_params.fb_base + (lcd_params.bbp * (lcd_params.hres * y_pos + x_pos))) = color;
    }
}

static void fb_fill_buffer(uint32_t* dest, uint32_t x_size, uint32_t y_size, uint32_t offset, uint32_t color) {
    ...
    if ( HAL_DMA2D_Init(&hlcd_dma2d) == HAL_OK ) {
        if ( HAL_DMA2D_ConfigLayer(&hlcd_dma2d, 1) == HAL_OK ) {
            if ( HAL_DMA2D_Start(&hlcd_dma2d, input_color, (uint32_t)dest, x_size, y_size) == HAL_OK ) {
...
void fb_fill_rect(uint32_t x_pos, uint32_t y_pos, uint32_t width, uint32_t height, uint32_t color) {
    ...
    uint32_t address = lcd_params.fb_base + ((lcd_params.bbp) * (lcd_params.hres * start_y + start_x));

    /* Fill the rectangle */
    fb_fill_buffer((uint32_t*)address, end_x - start_x, end_y - start_y, (lcd_params.hres - (end_x - start_x)), color);
}

void fb_draw_hline(uint32_t x_pos, uint32_t y_pos, uint32_t len, uint32_t color) {
    if ( y_pos > lcd_window.end_y || y_pos < lcd_window.start_y ) {
        return;
    }
    uint32_t address = lcd_params.fb_base + ((lcd_params.bbp) * (lcd_params.hres * y_pos + x_pos));
    fb_fill_buffer((uint32_t*)address, len, 1, 0, color);
}

void fb_draw_vline(uint32_t x_pos, uint32_t y_pos, uint32_t len, uint32_t color) {
    uint32_t address = lcd_params.fb_base + ((lcd_params.bbp) * (lcd_params.hres * y_pos + x_pos));
    fb_fill_buffer((uint32_t*)address, 1, len, lcd_params.hres - 1, color);
}

void dma2d_copy_buffer(uint32_t* pSrc, uint32_t* pDst, uint16_t x, uint16_t y, uint16_t xsize, uint16_t ysize) {
    uint32_t destination = (uint32_t)pDst + (y * lcd_params.hres + x) * (lcd_params.bbp);
...
void dma2d_copy_ycbcr_to_rgb(uint32_t* pSrc, uint32_t* pDst, uint16_t xsize, uint16_t ysize, uint32_t ChromaSampling) {
...
int DSI_DCS_read(uint16_t cmd, uint8_t* data, uint16_t data_len) {
    return HAL_DSI_Read(&hlcd_dsi, 0, data, data_len, DSI_DCS_SHORT_PKT_READ, cmd, (uint8_t[]){0, 0});
}

int DSI_DCS_write(uint16_t cmd, uint8_t* data, uint16_t data_len) {
    if ( data_len <= 1 ) {
        return HAL_DSI_ShortWrite(&hlcd_dsi, 0, DSI_DCS_SHORT_PKT_WRITE_P1, cmd, (uint32_t)data[0]);
    } else {
        return HAL_DSI_LongWrite(&hlcd_dsi, 0, DSI_DCS_LONG_PKT_WRITE, data_len, cmd, data);
    }
}
...
void lcd_init(void) {
    ...
    // ltdc
    {
        ...
        if ( ltdc_init(&hlcd_ltdc) != HAL_OK )
            dbg_printf("ltdc_init failed !\r\n");
