...
static const uint8_t cornertable[CORNER_RADIUS * CORNER_RADIUS] = {
    ...
    15, 15, 15, 0,  0,  0,  0,  0,  0,  0,  8,  15, 15, 15, 15, 15, 15, 15, 15, 0,  0,  0,  0,  0,  3,  12, 15, 15, 15,
    15, 15, 15, 15, 15, 15, 0,  0,  0,  0,  3,  14, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 0,  0,  0,  3,  14, 15, 15,
    15, 15, 15, 15, 15, 15, 15, 15, 15, 0,  0,  0,  12, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 0,  0,  8,  15,
    15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 0,  3,  15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 0,
    9,  15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 1,  15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15,
    15, 15, 5,  15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 9,  15, 15, 15, 15, 15, 15, 15, 15, 15, 15,
    15, 15, 15, 15, 15, 12, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 14, 15, 15, 15, 15, 15, 15, 15,
    15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15,
};

void display_bar_radius_ex(int x, int y, int w, int h, uint16_t c, uint16_t b, int r) {
...
void display_image(int x, int y, int w, int h, const void* data, uint32_t datalen) {
#if defined TREZOR_MODEL_T
    x += DISPLAY_OFFSET.x;
    y += DISPLAY_OFFSET.y;
    int x0 = 0, y0 = 0, x1 = 0, y1 = 0;
    ...
    uzlib_prepare(&decomp, decomp_window, data, datalen, decomp_out, sizeof(decomp_out));
    ...
    for ( uint32_t pos = 0; pos < w * h; pos++ ) {
        ...
        if ( px >= x0 && px <= x1 && py >= y0 && py <= y1 ) {
            fb_write_pixel(x_pos + px, y_pos + py, (decomp_out[0] << 8) | decomp_out[1]);
        }
...
void display_avatar(int x, int y, const void* data, uint32_t datalen, uint16_t fgcolor, uint16_t bgcolor) {
#if defined TREZOR_MODEL_T
    x += DISPLAY_OFFSET.x;
    y += DISPLAY_OFFSET.y;
    int x0 = 0, y0 = 0, x1 = 0, y1 = 0;
        ...
        if ( px >= x0 && px <= x1 && py >= y0 && py <= y1 ) {
            ...
            if ( d < AVATAR_BORDER_LOW ) {
                ...
                if ( d >= 16 ) {
                    ...
                    c = interpolate_color(fgcolor, (decomp_out[0] << 8) | decomp_out[1], d);
                }
...
void display_icon(int x, int y, int w, int h, const void* data, uint32_t datalen, uint16_t fgcolor, uint16_t bgcolor) {
    x += DISPLAY_OFFSET.x;
    y += DISPLAY_OFFSET.y;
...
bool display_toif_info(const uint8_t* data, uint32_t len, uint16_t* out_w, uint16_t* out_h, bool* out_grayscale) {
...
void display_loader_ex(
    ...
    display_set_window(
        DISPLAY_RESX / 2 - img_loader_size,
    ...
    if ( icon && memcmp(icon, "TOIg", 4) == 0 && LOADER_ICON_SIZE == *(uint16_t*)(icon + 4) &&
         LOADER_ICON_SIZE == *(uint16_t*)(icon + 6) && iconlen == 12 + *(uint32_t*)(icon + 8) ) {
        uint8_t icondata[(LOADER_ICON_SIZE * LOADER_ICON_SIZE) / 2] = {0};
        memzero(&icondata, sizeof(icondata));
        struct uzlib_uncomp decomp = {0};
        uzlib_prepare(&decomp, NULL, icon + 12, iconlen - 12, icondata, sizeof(icondata));
    ...
    #define LOADER_INDETERMINATE_WIDTH 100
            if ( icon && mx + my > (((LOADER_ICON_SIZE / 2) + LOADER_ICON_CORNER_CUT) * 2) &&
                 mx >= img_loader_size - (LOADER_ICON_SIZE / 2) && my >= img_loader_size - (LOADER_ICON_SIZE / 2) ) {
                int i = (x - (img_loader_size - (LOADER_ICON_SIZE / 2))) +
...
void display_print(const char* text, int textlen) {
    ...
    for ( int i = 0; i < textlen; i++ ) {
        ...
        case '\r':
            ...
            display_bar(0, DISPLAY_CHAR_HEIGHT * (row), DISPLAY_RESX, DISPLAY_CHAR_HEIGHT * 2, display_print_bgcolor);
            col = 0;
            width = 0;
            break;
        case '\n':
    ...
    for ( int y = 0; y < DISPLAY_PRINT_ROWS; y++ ) {
        if ( display_print_buf[y][0] != 0 ) {
            display_text(
                8,
                (y + 1) * DISPLAY_CHAR_HEIGHT,
                &display_print_buf[y][0],
                -1,
                FONT_NORMAL,
                display_print_fgcolor,
...
static void
display_text_render(int x, int y, const char* text, int textlen, int font, uint16_t fgcolor, uint16_t bgcolor) {
    // determine text length if not provided
    if ( textlen < 0 ) {
        textlen = strlen(text);
    }
...
int display_text_split(const char* text, int textlen, int font, int requested_width) {
    int width = 0;
    int lastspace = 0;
    // determine text length if not provided
    if ( textlen < 0 ) {
...
void display_text_printf(int x, int y, const char* fmt, ...) {
    if ( !strchr(fmt, '%') ) {
        display_text(x, y, fmt, strlen(fmt), FONT_NORMAL, display_print_fgcolor, display_print_bgcolor);
    } else {
        va_list va;
        va_start(va, fmt);
        char buf[256] = {0};
        int len = mini_vsnprintf(buf, sizeof(buf), fmt, va);
        display_text(x, y, buf, len, FONT_NORMAL, display_print_fgcolor, display_print_bgcolor);
        va_end(va);
    }
}
...
void display_qrcode(int x, int y, const char* data, uint32_t datalen, uint8_t scale) {
    if ( scale < 1 || scale > 10 )
        return;
...
void display_progress_percent(int x, int y, int permil) {
    ...
    display_text_center(x, y, (const char*)percent_asc, -1, FONT_PJKS_REGULAR_20, COLOR_WHITE, COLOR_BLACK);
#else
    display_bar(0, y - 28, MAX_DISPLAY_RESX, 28, COLOR_BLACK);
    display_text_center(x, y, (const char*)percent_asc, -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK);
#endif
}

void _display_progress(uint16_t y, const char* desc, int permil) {
    if ( desc ) {
        display_text_center(MAX_DISPLAY_RESX / 2, y + 50, desc, -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK);
    }
