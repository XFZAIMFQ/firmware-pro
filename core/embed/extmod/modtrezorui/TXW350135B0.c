...
int TXW350135B0_init_sequence(DSI_Writer_t dsi_writter, Delay_ms_uint32 delay_ms)
{
    ...
    DSI_CMD(dsi_writter, 0xB1, 0x0F, 0x18, 0x1E, 0x0C, 0x0F, 0x06, 0x08, 0x0A, 0x09, 0x24, 0x05, 0x10, 0x11,
    0x2A,0x34,
        0x1F // NVGAMCTRL
    );
    ...
    DSI_CMD(dsi_writter, 0xE5, 0x09, 0x2F, 0x2C, 0x8C, 0x0B, 0x31, 0x2C, 0x8C, 0x0D, 0x33, 0x2C, 0x8C, 0x0F, 0x35, 0x2C, 0x8C);
    DSI_CMD(dsi_writter, 0xE6, 0x00, 0x00, 0x33, 0x33); // ?
    DSI_CMD(dsi_writter, 0xE7, 0x44, 0x44);             // ?
    DSI_CMD(dsi_writter, 0xE8, 0x08, 0x2E, 0x2C, 0x8C, 0x0A, 0x30, 0x2C, 0x8C, 0x0C, 0x32, 0x2C, 0x8C, 0x0E, 0x34, 0x2C, 0x8C);
    DSI_CMD(dsi_writter, 0xE9, 0x36, 0x00);                               // ?
    DSI_CMD(dsi_writter, 0xEB, 0x00, 0x01, 0xE4, 0xE4, 0x44, 0x88, 0x40); // ?
    DSI_CMD(dsi_writter, 0xED, 0xFF, 0xFC, 0xB2, 0x45, 0x67, 0xFA, 0x01, 0xFF, 0xFF, 0x10, 0xAF, 0x76, 0x54, 0x2B,0xCF,0xFF);                                                              // ? DSI_CMD(dsi_writter, 0xef, 0x08, 0x08, 0x08, 0x45, 0x3f, 0x54); // ?
    ...
    DSI_CMD(dsi_writter, 0xff, 0x77, 0x01, 0x00, 0x00, 0x00); // CND2BKxSEL select bank0 without cmd2
    DSI_CMD(dsi_writter, 0x36, 0x00); // MADCTL normal scan, bgr->bgr
    // DSI_CMD(dsi_writter, 0x36, 0x10); // MADCTL normal scan, bgr->bgr
    ...
    HAL_Delay(20);                                 // delay
    DSI_CMD(dsi_writter, 0xff, 0x77, 0x01, 0x00, 0x00,0x10);                          // CND2BKxSEL select bank1 with
...
//     DSI_CMD(dsi_writter,0xc1, 0x14, 0x0C);
...
//     DSI_CMD(dsi_writter,0xb1, 0x4D);
//     DSI_CMD(dsi_writter,0xb2, 0x81);
//     DSI_CMD(dsi_writter,0xb3, 0x80);
...
//     DSI_CMD(dsi_writter,0x36, 0x00);

//     DSI_CMD(dsi_writter,0x29);
