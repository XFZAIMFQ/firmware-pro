...
static bool pn532_write_cmd_check_ack(pn532_controller_t* pn532, uint8_t cmd, uint8_t* params, uint8_t params_length) {
...
static bool pn532_read_response(pn532_controller_t* pn532, uint8_t* resonse, uint16_t* resonse_len, int timeout_ms) {
    pn532_frame_t frame = {0};
...
bool pn532_get_register(uint16_t reg_addr, uint8_t* value) {
    uint8_t command[3] = {PN532_COMMAND_READREGISTER, (uint8_t)((reg_addr >> 8) & 0xFF), (uint8_t)(reg_addr & 0xFF)};
    uint8_t response[1];
...
bool pn532_set_register(uint16_t reg_addr, uint8_t new_value) {
    uint8_t command[4] = {
        PN532_COMMAND_WRITEREGISTER, (uint8_t)((reg_addr >> 8) & 0xFF), (uint8_t)(reg_addr & 0xFF), new_value};
    uint8_t response[1];
    uint16_t response_length = sizeof(response);
